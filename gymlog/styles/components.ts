export const layout = {
  ...
  mainNarrow:  'w-full max-w-lg sm:max-w-xl md:max-w-2xl lg:max-w-4xl mx-auto px-3 sm:px-6 pt-3 pb-4',
  header:      'sticky top-0 bg-surface-0/90 backdrop-blur-md border-b border-surface-border z-20 pt-safe text-main',
  headerInner: 'w-full max-w-lg sm:max-w-xl md:max-w-2xl lg:max-w-4xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between',
  // Bento grid: 1 col → 2 col → 3 col → 4 col
  bentoGrid:   'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4',
  // Grid 2 columnas fijo (bento interior)
...
export const card = {
  ...
  // Card de receta
  recipe: 'bg-surface-1 border border-surface-border rounded-2xl p-4 sm:p-5 flex flex-col gap-2 hover:border-muted/50 transition-colors',
} as const
...
export const input = {
  ...
  small:  'bg-surface-2 border border-surface-border rounded-lg px-2 py-1.5 text-xs text-main placeholder-muted focus:outline-none focus:border-violet-500 w-full',
  center: 'bg-surface-2 border border-surface-border rounded-xl px-3 py-2.5 text-main placeholder-muted focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500/30 transition-colors duration-100 w-full text-center text-sm sm:text-base',
  // Input para cardio (distancia/tiempo)
  cardio: 'bg-surface-2 border border-surface-border rounded-xl px-3 py-2.5 text-main placeholder-muted focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500/30 transition-colors duration-100 w-full text-center font-mono text-base',
} as const

export const btn = {
  primary:      'bg-brand-500 hover:bg-brand-600 active:bg-brand-700 text-brand-foreground font-semibold rounded-xl px-4 py-2.5 transition-colors duration-100 select-none touch-manipulation text-sm sm:text-base',
  ghost:        'bg-surface-3 hover:bg-surface-4 active:bg-surface-4 text-main rounded-xl px-4 py-2.5 transition-colors duration-100 select-none touch-manipulation text-sm sm:text-base',
  ...
  chipInactive: 'bg-surface-3 text-muted hover:bg-surface-4 hover:text-main transition-colors',
  addDashed:    'w-full border-2 border-dashed border-surface-border hover:border-brand-500/50 hover:bg-brand-500/5 rounded-2xl py-4 flex items-center justify-center gap-2 text-muted hover:text-brand-400 transition-all duration-150 touch-manipulation',
  preset:       'flex-1 text-[11px] font-mono py-1 rounded-lg transition-colors touch-manipulation',
} as const

export const text = {
  label:        'text-[11px] sm:text-xs font-semibold text-muted uppercase tracking-widest',
  ...
  sectionTitle: 'font-semibold text-sm sm:text-base text-muted/80 mb-3 flex items-center gap-2',
...
export const setRow = {
  ...
  gridCardio:'grid grid-cols-4 gap-2 items-center text-sm py-1.5 px-1 rounded-lg hover:bg-surface-2 group transition-colors',
  header:    'grid grid-cols-4 gap-2 text-[11px] text-muted font-medium uppercase tracking-wider mb-2 px-1',
  index:     'text-muted font-mono',
  value:     'font-mono font-medium text-main',
  muted:     'text-muted text-xs',
  deleteBtn: 'opacity-0 group-hover:opacity-100 transition-opacity text-muted hover:text-red-400 p-1 -mr-1',
} as const
...
export const recipe = {
  ...
  ingredient:  'flex items-center gap-2 py-2 border-b border-surface-border last:border-0',
} as const

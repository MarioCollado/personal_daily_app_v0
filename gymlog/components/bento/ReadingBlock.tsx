    ...
    <div className="bento-card flex flex-col h-full relative overflow-hidden select-none [-webkit-touch-callout:none]" {...longPress}>
      {isLocked && (
        <div className="absolute inset-0 z-20 bg-surface-1/60 backdrop-blur-sm flex flex-col items-center justify-center p-4">
          <div className="w-10 h-10 lg:w-12 lg:h-12 rounded-full bg-violet-500/20 flex items-center justify-center mb-2">
            <Lock className="w-5 h-5 lg:w-6 lg:h-6 text-violet-400" />
          </div>
          <span className="text-violet-400 font-medium text-xs lg:text-sm">{t('dashboard.reading_block.locked')}</span>
          <span className="text-muted text-[10px] lg:text-xs mt-1 text-center font-medium">{t('dashboard.reading_block.locked_desc')}</span>
        </div>
      )}

      <div className="relative flex items-center justify-center mb-2 lg:mb-3 min-h-[20px]">
        <div className="flex items-center gap-1.5">
          <BookOpen className="w-3.5 h-3.5 lg:w-4 lg:h-4 text-violet-400" />
          <span className="text-[11px] lg:text-xs font-bold text-muted uppercase tracking-widest">{t('dashboard.reading_block.title')}</span>
        </div>
        ...
        {editingBook ? (
          <>
            <input
              ...
              className="w-full bg-surface-2 border border-surface-border rounded-lg px-2 lg:px-3 py-1.5 lg:py-2 text-xs lg:text-sm text-main placeholder-muted focus:outline-none focus:border-violet-500"
            />
              ...
              <div className="absolute top-full left-0 right-0 mt-0.5 bg-surface-2 border border-surface-border rounded-lg overflow-hidden z-10 shadow-xl">
                {filtered.slice(0, 4).map(s => (
                  <button key={s} onMouseDown={() => { setBookInput(s); onChange({ book_title: s }); setEditingBook(false); setShowSuggestions(false) }}
                    className="w-full text-left px-3 lg:px-4 py-2 lg:py-2.5 text-xs lg:text-sm hover:bg-surface-3 text-main transition-colors border-b border-surface-border last:border-0 truncate">
                    {s}
                  </button>
                ))}
              </div>
          ...
          <button onClick={() => { setEditingBook(true); setBookInput(bookTitle || '') }}
            className={clsx('w-full text-left text-xs lg:text-sm rounded-lg px-2.5 lg:px-3 py-2 lg:py-2.5 transition-colors hover:bg-surface-2 group flex items-center justify-between gap-1',
            ...
            <ChevronDown className="w-3 h-3 lg:w-3.5 lg:h-3.5 text-muted flex-shrink-0 opacity-0 group-hover:opacity-100" />
          </button>
        )}
      </div>

      <div className="flex items-center gap-2 mb-3 lg:mb-4 px-1">
        <button onClick={() => addPages(-5)} className="w-8 h-8 lg:w-9 lg:h-9 rounded-lg bg-surface-2 hover:bg-surface-3 flex items-center justify-center text-muted hover:text-main transition-colors touch-manipulation">
          <Minus className="w-3.5 h-3.5 lg:w-4 lg:h-4" />
        </button>
        <div className="flex-1 text-center">
          <span className="text-2xl lg:text-3xl font-mono font-bold text-main leading-none">{pagesRead ?? 0}</span>
          <div className="text-[10px] lg:text-xs text-muted font-bold uppercase tracking-tighter mt-1">{t('dashboard.reading_block.pages_today')}</div>
        </div>
        <button onClick={() => addPages(5)} className="w-8 h-8 lg:w-9 lg:h-9 rounded-lg bg-surface-2 hover:bg-surface-3 flex items-center justify-center text-muted hover:text-main transition-colors touch-manipulation">
      ...
      <div className="flex gap-1.5 mb-4 lg:mb-5">
        {[1, 10, 25, 50].map(n => (
          <button key={n} onClick={() => addPages(n)}
            className="flex-1 text-[11px] lg:text-xs font-mono font-bold py-1.5 lg:py-2 rounded-lg bg-surface-2 hover:bg-violet-500/20 hover:text-violet-400 text-muted transition-all duration-200 touch-manipulation">
            +{n}
          </button>
        ))}
      </div>
      ...
      {bookTitle && bookTotalPages && (
        <div className="px-1">
          <div className="flex justify-between text-[10px] lg:text-xs font-bold text-muted mb-1.5 lg:mb-2 uppercase tracking-tighter">
            <span>{t('dashboard.reading_block.total_pages', { current: accumulatedPages ?? pagesRead ?? 0, total: bookTotalPages })}</span>
            <span>{progress ?? 0}%</span>
          </div>
          <div className="h-1.5 lg:h-2 bg-surface-3 rounded-full overflow-hidden">
      ...
      {bookTitle && !bookTotalPages && (
        <button
          ...
          className="text-[10px] lg:text-xs font-bold uppercase tracking-tighter text-muted hover:text-violet-400 transition-colors text-center w-full mt-2">

...
const IS_CARDIO = (muscleGroup: string | null) => muscleGroup?.toLowerCase() === 'cardio'
...
export default function ExerciseCard({ exercise, userId, onDelete, onSetAdded, onSetDeleted }: Props) {
  ...
  useEffect(() => {
    if (!expanded || isCardio || !userId || previousLookupDone) return
  ...
  useEffect(() => {
    if (isCardio || !expanded || !previousSuggestion) return
  ...
  async function handleAddSet() {
    ...
    if (isCardio) {
      ...
      const safeDistance = distanceNumber > 0 ? distanceNumber : 0
      setAdding(true)

      try {
        const set = await addSet(exercise.id, 0, 0, undefined, undefined, safeDistance || undefined, durationSeconds)

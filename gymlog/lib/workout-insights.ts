...
export function getWorkoutLoadValue(exercises: Exercise[]) {
  ...
  const total = exercises.reduce(
    (sum, exercise) => sum + getExerciseStimulus(exercise),
    0,
  );
  return Math.round(total);
}

export function buildMuscleActivation(
  exercises: Exercise[],
): MuscleActivation[] {
  ...
  for (const exercise of exercises) {
    ...
    const stimulus =
      baseStimulus > 0 ? baseStimulus : exercise.sets?.length ? 0 : 0.65;
    if (stimulus <= 0) continue;

    for (const target of distribution) {
      rawBySlug.set(
        target.slug,
        (rawBySlug.get(target.slug) || 0) + stimulus * target.weight,
      );
    }
  }
  ...
  return Array.from(rawBySlug.entries())
    .map(([slug, raw]) => {
      ...
      const intensity: 1 | 2 | 3 =
        normalized >= 0.72 ? 3 : normalized >= 0.38 ? 2 : 1;
      const level: MuscleActivationLevel =
        intensity === 3 ? "high" : intensity === 2 ? "medium" : "low";
...
export function getBodyHighlighterData(
  exercises: Exercise[],
): ExtendedBodyPart[] {
      ...
      const value =
        metric === "sessions"
          ...
          : workoutsForDay.reduce(
              (sum, workout) =>
                sum + getWorkoutLoadValue(workout.exercises || []),
              0,
            );
